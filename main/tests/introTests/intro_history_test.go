...
func (s *MyFirstSuite) TestCreateHistoryIntro(t provider.T) {
	tests := []struct {
		nameTest  string
	...
	for _, test := range tests {
		t.Run(test.nameTest, func(t provider.T) {
			...
			idHistory, err := services.IHistoryDevice.CreateDeviceHistory(clientID, newHistory.AddHistory)
			t.Require().NoError(err)
...
func (s *MyFirstSuite) TestGetDeviceHistoryIntro(t provider.T) {
	tests := []struct {
		nameTest string
		lenList  int
		user     factory.ObjectSystem
	...
	for _, test := range tests {
		t.Run(test.nameTest, func(t provider.T) {
			...
			resultListHistory, err := services.IHistoryDevice.GetDeviceHistory(clientID, newDevice.Name, newHome.Name)
